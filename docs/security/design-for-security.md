...
| THR-008 | Required target result is inferred from another target | Conformance and availability | Per-target Pass/Fail/Unknown profile and retained native output | Candidate ARM64 native evidence is currently Unknown |
...
| Native ARM64 candidate evidence unavailable | Cross-build cannot prove runtime behavior | Maintainer | None | Exact-candidate native run |
