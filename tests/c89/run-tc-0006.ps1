...
$buildDirectory = Join-Path $repoRoot 'build\tests\c89\tc-0006'
$executable = Join-Path $buildDirectory 'locale-test.exe'
...
$arguments = @(
    ...
    (Join-Path $repoRoot 'tests\c89\locale.c'),
    '-o', $executable
)
$buildOutput = & $TinyCc @arguments 2>&1
if ($LASTEXITCODE -ne 0) {
    throw "TC-0006 build failed:`n$($buildOutput | Out-String)"
}

& $executable
if ($LASTEXITCODE -ne 0) {
    throw "TC-0006 behavior failed with code $LASTEXITCODE."
}
...
[PSCustomObject]@{
    ...
    StartupLocale = 'C'
    CategoryQueries = 'Pass'
    InvalidRequests = 'Pass'
    LocaleConventions = 'Pass'
    CtypeInteraction = 'Pass'
    ExitCode = 0
}
