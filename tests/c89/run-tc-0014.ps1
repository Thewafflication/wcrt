...
$buildDirectory = Join-Path $repoRoot 'build\tests\c89\tc-0014'
$executable = Join-Path $buildDirectory 'string-test.exe'
...
$arguments = @(
    ...
    (Join-Path $repoRoot 'tests\c89\string.c'),
    '-o', $executable
)
$buildOutput = & $TinyCc @arguments 2>&1
if ($LASTEXITCODE -ne 0) {
    throw "TC-0014 build failed:`n$($buildOutput | Out-String)"
}

& $executable strict-wcrt-messages
if ($LASTEXITCODE -ne 0) {
    throw "TC-0014 behavior failed with code $LASTEXITCODE."
}
...
[PSCustomObject]@{
    ...
    FunctionsCovered = 22
    CopyAndOverlap = 'Pass'
    BoundsAndPadding = 'Pass'
    UnsignedComparison = 'Pass'
    SearchAndTerminators = 'Pass'
    LocaleCCollation = 'Pass'
    TokenState = 'Pass'
    ErrorMessages = 'Pass'
    ExitCode = 0
}
