...
$buildDirectory = Join-Path $repoRoot 'build\tests\c89\tc-0004'
$executable = Join-Path $buildDirectory 'float-test.exe'
...
$arguments = @(
    ...
    (Join-Path $repoRoot 'tests\c89\float.c'),
    '-o', $executable
)
$buildOutput = & $TinyCc @arguments 2>&1
if ($LASTEXITCODE -ne 0) {
    throw "TC-0004 build failed:`n$($buildOutput | Out-String)"
}

& $executable
if ($LASTEXITCODE -ne 0) {
    throw "TC-0004 behavior failed with code $LASTEXITCODE."
}
...
[PSCustomObject]@{
    ...
    Radix = 2
    FloatModel = 'binary32'
    DoubleModel = 'binary64'
    LongDoubleMantissaBits = 53
    EpsilonBehavior = 'Pass'
    RangeConstraints = 'Pass'
    ExitCode = 0
}
