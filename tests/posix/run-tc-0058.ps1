...
$build = Join-Path $root 'build\tests\posix\tc-0058'
...
$fixtures = @(
    @{Name = 'selected'; Path = 'presence\strings.c'},
    @{Name = 'strict'; Path = 'absence\strings.c'}
)
foreach ($mode in 'c89', 'c99') {
    foreach ($fixture in $fixtures) {
        ...
        if ($LASTEXITCODE -ne 0) {
            throw "TC-0058 $($fixture.Name) $mode compile failed:`n" +
                ($output -join "`n")
        }
    }
}
...
$behavior = Join-Path $PSScriptRoot 'strings.c'
$output = @(& $TinyCc -std=c89 -Wall -Werror -I $include @sources `
    $behavior -o $executable 2>&1)
if ($LASTEXITCODE -ne 0) {
    throw "TC-0058 behavior build failed:`n$($output -join "`n")"
}
& $executable
if ($LASTEXITCODE -ne 0) {
    throw "TC-0058 behavior failed with code $LASTEXITCODE."
}
[PSCustomObject]@{TestCase = 'TC-0058'; Requirement = 'REQ-0058';
    Status = 'Pass'}
