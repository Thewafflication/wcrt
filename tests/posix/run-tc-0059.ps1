...
$build = Join-Path $root 'build\tests\posix\tc-0059'
...
$fixtures = @(
    @{Name = 'selected'; Path = 'presence\strdup.c'},
    @{Name = 'strict'; Path = 'absence\strdup.c'}
)
foreach ($mode in 'c89', 'c99') {
    foreach ($fixture in $fixtures) {
        ...
        if ($LASTEXITCODE -ne 0) {
            throw "TC-0059 $($fixture.Name) $mode compile failed:`n" +
                ($output -join "`n")
        }
    }
}
...
$behavior = Join-Path $PSScriptRoot 'strdup.c'
$output = @(& $TinyCc -std=c89 -Wall -Werror -I $include @sources `
    $behavior -o $executable 2>&1)
if ($LASTEXITCODE -ne 0) {
    throw "TC-0059 behavior build failed:`n$($output -join "`n")"
}
& $executable
if ($LASTEXITCODE -ne 0) {
    throw "TC-0059 behavior failed with code $LASTEXITCODE."
}
[PSCustomObject]@{TestCase = 'TC-0059'; Requirement = 'REQ-0059';
    Status = 'Pass'}
