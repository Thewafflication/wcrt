...
$build = Join-Path $root 'build\tests\posix\tc-0060'
...
$fixtures = @(
    @{Name = 'selected'; Path = 'presence\unistd.c'},
    @{Name = 'strict'; Path = 'absence\unistd.c'}
)
foreach ($mode in 'c89', 'c99') {
    foreach ($fixture in $fixtures) {
        ...
        if ($LASTEXITCODE -ne 0) {
            throw "TC-0060 $($fixture.Name) $mode compile failed:`n" +
                ($output -join "`n")
        }
    }
}
...
$behavior = Join-Path $PSScriptRoot 'unistd.c'
$output = @(& $TinyCc -std=c89 -Wall -Werror -I $include @sources `
    $behavior -o $executable 2>&1)
if ($LASTEXITCODE -ne 0) {
    throw "TC-0060 behavior build failed:`n$($output -join "`n")"
}
& $executable
if ($LASTEXITCODE -ne 0) {
    throw "TC-0060 behavior failed with code $LASTEXITCODE."
}
[PSCustomObject]@{TestCase = 'TC-0060'; Requirement = 'REQ-0060';
    Status = 'Pass'}
