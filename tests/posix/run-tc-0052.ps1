...
$build = Join-Path $root 'build\tests\posix\tc-0052'
...
$fixtures = @(
    @{Name = 'selected'; Path = 'presence\dirent.c'},
    @{Name = 'strict'; Path = 'absence\dirent.c'}
)
foreach ($mode in 'c89', 'c99') {
    foreach ($fixture in $fixtures) {
        ...
        if ($LASTEXITCODE -ne 0) {
            throw "TC-0052 $($fixture.Name) $mode compile failed:`n" +
                ($output -join "`n")
        }
    }
}
...
$behavior = Join-Path $PSScriptRoot 'dirent.c'
$output = @(& $TinyCc -std=c89 -Wall -Werror -I $include @sources `
    $behavior -o $executable 2>&1)
if ($LASTEXITCODE -ne 0) {
    throw "TC-0052 behavior build failed:`n$($output -join "`n")"
}
& $executable $build
if ($LASTEXITCODE -ne 0) {
    throw "TC-0052 behavior failed with code $LASTEXITCODE."
}
[PSCustomObject]@{TestCase = 'TC-0052'; Requirement = 'REQ-0052';
    Status = 'Pass'}
