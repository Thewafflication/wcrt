...
$build = Join-Path $root 'build\tests\posix\tc-0066'
...
foreach ($mode in 'c89', 'c99') {
    foreach ($fixture in 'presence\timeval.c', 'absence\timeval.c') {
        $name = [IO.Path]::GetFileName((Split-Path -Parent $fixture))
        $object = Join-Path $build "$name-$mode.o"
        $output = @(& $TinyCc "-std=$mode" -Wall -Werror -nostdinc `
            -I $include -I $tinyCcInclude -c `
            (Join-Path $PSScriptRoot $fixture) -o $object 2>&1)
        if ($LASTEXITCODE -ne 0) {
            throw "TC-0066 $name $mode compile failed:`n$($output -join "`n")"
        }
    }
}
...
$output = @(& $TinyCc -std=c89 -Wall -Werror -I $include @sources `
    (Join-Path $PSScriptRoot 'timeval.c') -o $executable 2>&1)
if ($LASTEXITCODE -ne 0) {
    throw "TC-0066 behavior build failed:`n$($output -join "`n")"
}
& $executable
if ($LASTEXITCODE -ne 0) {
    throw "TC-0066 behavior failed with code $LASTEXITCODE."
}
[PSCustomObject]@{TestCase = 'TC-0066'; Requirement = 'REQ-0066';
    Status = 'Pass'}
