...
$build = Join-Path $root 'build\tests\posix\tc-0065'
...
$fixtures = @(
    @{Name = 'selected'; Path = 'presence\paths.c'},
    @{Name = 'strict'; Path = 'absence\paths.c'}
)
foreach ($mode in 'c89', 'c99') {
    foreach ($fixture in $fixtures) {
        ...
        if ($LASTEXITCODE -ne 0) {
            throw "TC-0065 $($fixture.Name) $mode compile failed:`n" +
                ($output -join "`n")
        }
    }
}
...
$behavior = Join-Path $PSScriptRoot 'paths.c'
$path = Join-Path $build 'access-target.txt'
$directory = Join-Path $build 'created'
$missing = Join-Path $build 'missing-parent\created'
$output = @(& $TinyCc -std=c89 -Wall -Werror -I $include @sources `
    $behavior -o $executable 2>&1)
if ($LASTEXITCODE -ne 0) {
    throw "TC-0065 behavior build failed:`n$($output -join "`n")"
}
try {
    & $executable $path $directory $missing
    if ($LASTEXITCODE -ne 0) {
        throw "TC-0065 behavior failed with code $LASTEXITCODE."
    }
} finally {
...
[PSCustomObject]@{TestCase = 'TC-0065'; Requirement = 'REQ-0065';
    Status = 'Pass'}
