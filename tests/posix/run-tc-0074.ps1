...
foreach ($test in @('stdio_slots', 'stdio_threads')) {
    $executable = Join-Path $build "$test.exe"
    & $TinyCc -std=c89 -Wall -Werror -I $include @sources `
        (Join-Path $PSScriptRoot "$test.c") -o $executable
    if ($LASTEXITCODE -ne 0) { throw "TC-0074 $test build failed." }
    Push-Location $build
    try {
        & $executable
        if ($LASTEXITCODE -ne 0) {
            throw "TC-0074 $test failed with $LASTEXITCODE."
        }
    } finally {
        Pop-Location
    }
