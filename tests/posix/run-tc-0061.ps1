...
$build = Join-Path $root 'build\tests\posix\tc-0061'
...
$fixtures = @(
    @{Name = 'selected'; Path = 'presence\unlink.c'},
    @{Name = 'strict'; Path = 'absence\unlink.c'}
)
foreach ($mode in 'c89', 'c99') {
    foreach ($fixture in $fixtures) {
        ...
        if ($LASTEXITCODE -ne 0) {
            throw "TC-0061 $($fixture.Name) $mode compile failed:`n" +
                ($output -join "`n")
        }
    }
}
...
$behavior = Join-Path $PSScriptRoot 'unlink.c'
$path = Join-Path $build 'unlink-target.txt'
$output = @(& $TinyCc -std=c89 -Wall -Werror -I $include @sources `
    $behavior -o $executable 2>&1)
if ($LASTEXITCODE -ne 0) {
    throw "TC-0061 behavior build failed:`n$($output -join "`n")"
}
& $executable $path
if ($LASTEXITCODE -ne 0) {
    throw "TC-0061 behavior failed with code $LASTEXITCODE."
}
[PSCustomObject]@{TestCase = 'TC-0061'; Requirement = 'REQ-0061';
    Status = 'Pass'}
